...
export async function deleteComponent(id: number) {
  ...
  if (component) {
    ...
    if (
      ...
      component.preview_url.includes("component-previews/")
    ) {
    ...
    if (
      ...
      component.thumbnail_url.includes("component-previews/")
    ) {
...
export async function incrementViewCount(componentId: number) {
  const supabase = await createClient();
  const { error } = await supabase.rpc("increment_view_count", {
    component_id_param: componentId,
  });
